...
import com.denizozcan.matchme.databinding.SingleplayerBinding
...
class SinglePlayer : AppCompatActivity() {
    ...
    var invisible = ArrayList<ImageView>()
    ...
    private fun startTimer(){
        cDT = object  : CountDownTimer(45500,1000){
            override fun onFinish() {
                if(!activeness){
                    applicationContext.stopService(Intent(applicationContext, SoundService::class.java))
                    binding.timeText.text = "Time: 0"
                    handler.removeCallbacks(runnable)
                    val alert = AlertDialog.Builder(this@SinglePlayer,R.style.AlertDialog)
            ...
            override fun onTick(millisUntilFinished: Long) {
                if(millisUntilFinished/1000<=10){
                    binding.timeText.setTextColor(Color.parseColor("#F50057"))
                }
                if(!activeness){
                    binding.timeText.text = "Time: ${millisUntilFinished/1000}"
                    if(score>=0){ binding.scoreText.setTextColor(Color.parseColor("#1DE9B6")) }else{ binding.scoreText.setTextColor(Color.parseColor("#F50057")) }
                }
            }
        }.start()
    }
    ...
    private fun openingAnimation(imageView: ImageView, card: Card,i: Int){
        if (!activeness) {
            if (!continuality) {
                ...
                val animation = AnimationUtils.loadAnimation(this@SinglePlayer, R.anim.opencard)
                imageView.startAnimation(animation)
                animation.setAnimationListener(object : Animation.AnimationListener {
                    override fun onAnimationStart(animation: Animation?) { continuality = true }
                    override fun onAnimationEnd(animation: Animation?) {
                        val animation2 = AnimationUtils.loadAnimation(this@SinglePlayer, R.anim.closecard)
                            ...
                            override fun onAnimationEnd(animation: Animation?) {
                                ...
                                if (opened.size == 2) {
                                    if (opened[0].n == opened[1].n) {
                                        ...
                                        val time = binding.timeText.text.toString().replace("Time: ","").toInt()
                                        score += (2*opened[0].p*opened[0].h)*(time/10)
                                        ...
                                        if(counter == allCards.size) {
                                            ...
                                            alert.setMessage("You Won the game. Do you want to try again?")
                                            alert.setPositiveButton("") {_, _ ->
                                                cDT.cancel()
                                                startActivity(intent)
                                                finish()
                                        ...
                                        val time = binding.timeText.text.toString().replace("Time: ","").toInt()
                                        score -= ((opened[0].p+opened[1].p)/opened[0].h)*(time/10)
                                        binding.scoreText.text = "Score: $score"
                                        closingAnimation(allCards[opened[0].i], opened[0].i)
                                        closingAnimation(allCards[opened[1].i], opened[1].i)
                                    } else {
                                        val time = binding.timeText.text.toString().replace("Time: ","").toInt()
                                        score -= ((opened[0].p+opened[1].p)/2)*opened[0].h*opened[1].h*(time/10)
                                        binding.scoreText.text = "Score: $score"
                                        closingAnimation(allCards[opened[0].i], opened[0].i)
                                        closingAnimation(allCards[opened[1].i], opened[1].i)
                                    }
        ...
        if(opened.size==0){
            object  : CountDownTimer(1000,1000){
                ...
                override fun onFinish() {
                    for (card in mapping) {
                        if(card.key.visibility==View.VISIBLE){
                            val animation3 = AnimationUtils.loadAnimation(this@SinglePlayer, R.anim.opencard)
                            card.key.startAnimation(animation3)
                            animation3.setAnimationListener(object : Animation.AnimationListener {
                                override fun onAnimationStart(animation: Animation?) {}
                                override fun onAnimationEnd(animation: Animation?){
                                    val animation4 = AnimationUtils.loadAnimation(this@SinglePlayer, R.anim.closecard)
